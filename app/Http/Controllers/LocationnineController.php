...
class LocationnineController extends Controller
{
    public function LocationCustomize(Request $request)
    {
        ...
        try {
            if (Locatoincustomize::where('location_com_id', Auth::user()->com_id)->exists()) {
                ...
                $location->location9 = $request->location9;
                $location->save();
    ...
    {
        $validated = $request->validate([
            'location_nine_name' => 'required',
        ]);
        try {
            $location = new Locationnine();
            $location->location_nine_com_id = Auth::user()->com_id;
            $location->location_nine_region_id = $request->region_id;
            $location->location_nine_area_id = $request->area_id;
            $location->location_nine_territory_id = $request->territory_id;
            $location->location_nine_town_id = $request->town_id;
            $location->location_nine_db_house_id = $request->db_house_id;
            $location->location_nine_six_id = $request->location_six_id;
            $location->location_nine_seven_id = $request->location_seven_id;
            $location->location_nine_eight_id = $request->location_eight_id;
            $location->location_nine_name = $request->location_nine_name;
            $location->save();
        } catch (\Exception $e) {
    ...
    public function updateLocation(Request $request)
    {
        ...
        try {
            $location =  Locationnine::find($request->id);
            $location->location_nine_com_id = Auth::user()->com_id;
            $location->location_nine_region_id = $request->region_id;
            $location->location_nine_area_id = $request->area_id;
            $location->location_nine_territory_id = $request->territory_id;
        ...
        return redirect('location-nine')->with('message', 'Location Updated Successfully');
    }


    public function   editLocationNine($id)
    {

        $locations_nine = Locationnine::where('id', '=', $id)->get();
        ...
        return view('back-end.premium.organization.locationnine.locationnine-edit', get_defined_vars())
    ...
    public function deleteLocation($id)
    {
        ...
        DB::beginTransaction();
        try {
            $location = Locationnine::where('id', $id)->delete();
    ...
    {
        try {
            $location = Locationnine::where('region_com_id', $request->bulk_delete_com_id)->delete();
            return back()->with('message', 'Deleted Successfully');
        } catch (\Exception $e) {
