<div class="container-fluid">
    ...
    @foreach ($data as $item)
        @php
            ...
            $employee_id = User::where('id', $item->objective_emp_id)->first(['id']);
            ...
            $userName = valueTypeConfig::with([
                'valueUser' => function ($q) {
                ...
                ->first();
            $detailsDevelopment = DevelopmentPlan::where('development_emp_id', $item->objective_emp_id)
                ->with('developmentPlanDetails')
                ->first();
        @endphp

        <div class="performance-report-pdf" style="page-break-after: always;">
            <div class="header-info">
                <div class="header-logo">
                    <img height="50" width="100px" src="{{ public_path($company_logo->company_logo) }}" />
                    <p style="text-align: right">STRICTLY CONFIDENTIAL WHEN COMPLETED</p>
                </div>
            ...
            <table style="width: 100%">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ ucfirst($item->userfromobjective->first_name) }}
                            {{ ucfirst($item->userfromobjective->last_name) }}</td>
                        <td>PIN:</td>
                        <td colspan="2">{{ $item->userfromobjective->company_assigned_id }}
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">Designation in use (including function):</td>
                        <td>{{ $item->userdesignationfromobjective->designation_name }}</td>
                        <td colspan="2">Last Performance related salary increase Date:</td>
                        <td>
                            @if ($item->userfromobjective->salaryIncrement)
                                @if ($item->userfromobjective->salaryIncrement->salary_history_approval_status == 1)
                                    {{ $item->userfromobjective->salaryIncrement->salary_history_increment_date }}
                                @else
                                    {{ 'Not Applicable' }}
                                @endif
                            @endif
                    ...
                    <tr>
                        ...
                        <td>{{ $item->userfromobjective->promotion->promotion_date ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">Date of joining BRAC:</td>
                        <td>{{ $item->userfromobjective->joining_date }}</td>
                        <td>Country:</td>
                        <td colspan="2">
                            {{ $item->userfromobjective->emoloyeedetail->userNationality->name ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>Base Location:</td>
                        <td colspan="2">
                            {{ $item->userfromobjective->userregion->region_name ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">
                        ...
                        <td>{{ $item->userfromobjective->qualification->qualification_education_level ?? 'Not Selected' }}
                        </td>
                        <td colspan="2">Contract/Regular:</td>
                        <td>{{ $item->userfromobjective->job_nature ?? '' }}</td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            @if ($item->userfromobjective->employment_type == 'Contactual')
                        <td scope="col">
                            {{ $item->userfromobjective->expiry_date }}</td>
                    @else
                        {{ 'Not Applicable' }}
    @endif
    </td>
    ...
    <tr>
        ...
        <td colspan="2">Last Contract Renewal Date:</td>
        <td>
            @if ($item->userfromobjective->employment_type == 'Contactual')
        <td scope="col">
            {{ $item->userfromobjective->expiry_date }}</td>
    ...
    <tr>
        ...
        <td colspan="2">Supervisor PIN:</td>
    ...
    <table style="width: 100%">
        <tbody>
            ...
            @foreach ($item->objectiveFilter->unique(fn($p) => $p->objectiveTypeConfig->userobjectivetypefromobjectiveconfig->objective_type_name) as $objective)
                <?php //dd($objective);
                ?>
                <?php
            ...
            @if ($detailsDevelopment)
                ...
                @foreach ($detailsDevelopment->developmentPlanDetails as $value)
                    <tr>
                        ...
                        <td colspan="3"> {{ $value->development_action_taken }}
                        </td>
                    </tr>
                @endforeach
    ...
    <table style="width: 100%">
        <tbody>
            ...
            @foreach ($item->objectiveFilter->unique(fn($p) => $p->objectiveTypeConfig->userobjectivetypefromobjectiveconfig->objective_type_name) as $objective)
                ...
                <tr>
                    <th colspan="6" style="background-color: #04AA6D; text-aligh:center;">
                        {{ $objective->objectiveTypeConfig->userobjectivetypefromobjectiveconfig->objective_type_name ?? null }}
                        -
                        {{ $objective->objectiveTypeConfig->obj_config_percent ?? null }}%
                    </th>
            ...
            <tr>
                <td>
                    <div class="singnature">
                        <p>Employee Name: {{ $item->userfromobjective->first_name }}
                            {{ $item->userfromobjective->last_name }}</p>
                    </div>
                </td>
                <td>
                    <div class="singnature">
        ...
        <tbody>
            @if (!$value_type_config_details->isEmpty())
                ...
                @foreach ($value_type_config_details->unique(fn($p) => $p->valuetype->value_type_name) as $variable_type)
                    ...
                    ?>
                    <tr>
                        <th colspan="6" style="background-color: #04AA6D">
